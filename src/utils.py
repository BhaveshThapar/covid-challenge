...
def compute_per_source_f1(y_true, y_pred, sources):
    ...
    Per-source F1: average of per-class F1 scores, but only for classes
    that have at least one ground-truth sample in that source. If a source
    has no covid ground-truth, its F1 is just F1_noncovid (and vice versa).
    ...
    for src in unique_sources:
        ...
        if mask.sum() == 0:
            continue
        labels_present = np.unique(y_true[mask])  # only ground-truth classes
        f1 = f1_score(y_true[mask], y_pred[mask], average="macro", zero_division=0, labels=labels_present)
